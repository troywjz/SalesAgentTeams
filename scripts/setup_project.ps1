...
if (-not (Test-Path -LiteralPath ".env")) {
    ...
    Write-Host "已创建 .env。请填写 DEEPSEEK_API_KEY、ALIYUN_API_KEY 和 SILICONFLOW_API_KEY；该文件不会提交到 Git。" -ForegroundColor Yellow
} else {
...
Write-Host "1. 在 .env 中填写 DeepSeek、阿里云和 SiliconFlow 三个 API Key。"
